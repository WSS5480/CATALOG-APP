...
import pwa                       # install to home screen: manifest, worker, icons
...
PUBLIC_PATHS = ("/login", "/login.html", "/api/auth/", "/healthz", "/favicon.ico",
                "/app.css", "/static/",
                # The install files. A phone asks for these before anyone has
                # signed in; sending them to the sign-in page instead would make
                # the app look uninstallable rather than locked.
...
async def login_page():
    try:
        with open("static/login.html", "rb") as fh:
            # The sign-in page is where most people land, so it is where the
            # install prompt has to be offered.
            return Response(pwa.inject(fh.read()), media_type="text/html")
...
app.include_router(pwa.router)   # must come before the catch-all static mount
